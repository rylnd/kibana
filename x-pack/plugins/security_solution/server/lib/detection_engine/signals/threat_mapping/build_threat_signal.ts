...
export const buildThreatSignal = async ({
  ...
  currentThreatList,
  currentResult,
}: BuildThreatSignalOptions): Promise<RuleQueryResult> => {
  ...
  if (threatFilter.query.bool.should.length === 0) {
    ...
    const result = await performRuleQuery({
      gap,
      previousStartedAt,
      listClient,
