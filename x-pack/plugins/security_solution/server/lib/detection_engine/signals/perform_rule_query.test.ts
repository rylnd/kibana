...
import { performRuleQuery } from './perform_rule_query';
...
describe('performRuleQuery', () => {
  ...
  test('should return success with number of searches less than max signals', async () => {
    ...
    const { success, createdSignalsCount, lastLookBackDate } = await performRuleQuery({
  ...
  test.skip('if unsuccessful first bulk create', async () => {
    const exceptionItem = getExceptionListItemSchemaMock();
    exceptionItem.entries = [
      {
        field: 'source.ip',
  ...
  test.skip('it returns error array when singleSearchAfter returns errors', async () => {
    const sampleParams = sampleRuleAlertParams(30);
    ...
    const { success, createdSignalsCount, lastLookBackDate, errors } = await performRuleQuery({
      ruleParams: sampleParams,
      gap: null,
      previousStartedAt: new Date(),
      listClient,
